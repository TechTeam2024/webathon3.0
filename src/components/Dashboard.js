...
function Dashboard() {
    ...
    useEffect(() => {
        (async () => {
            const jury = localStorage.getItem("jury_no")
            const response = await axios.get(URL)
            const data = response.data.filter(submission => jury == 0 ? submission : submission.jury == jury && submission.evaluated == 0)
            console.log(jury)
            setSubmissions(data)
            console.log(data)
        })()
            ...
            <header className="shadow-sm">
                <div className="max-w-7xl mx-auto">
                    <div className="flex justify-between">
                        ...
                        <button className="btn bg-indigo-600 w-[120px] h-[40px] rounded-lg"
                            onClick={() => {
                                localStorage.clear()
